...
public class DossierLocalServiceImpl extends DossierLocalServiceBaseImpl {
	...
	/**
	 * <p> Lấy về thông tin một hồ sơ </p>
	 * 
	 * @param receptionNo
	 *            là mã số tiếp nhận hồ sơ
	 * @return trả về đối tượng Dossier theo receptionNo
	...
	public Dossier getDossierByReceptionNo(String receptionNo)
		...
		return dossierPersistence.findByReceptionNo(receptionNo);
	}

	/**
	 * <p> Tìm kiếm các hồ sơ theo từ khóa </p>
	 * 
	 * @param groupId
	 * @param keyword
	 *            là từ khóa tìm kiếm hồ sơ
	 * @return trả về danh sách các đối tượng Dossier theo từ khóa
	 * @throws SystemException
	 *             Nếu ngoại lệ hệ thống xảy ra
	 * @throws NoSuchDossierException
	 *             Khi xảy ra lỗi không tìm thấy Dossier
	...
	public List<Dossier> searchDossier(
		...
		return dossierFinder.searchDossier(groupId, keywords, start, end);
	}

	/**
	 * <p> Đếm số lượng các hồ sơ theo từ khóa </p>
	 * 
	 * @param groupId
	 * @param keyword
	 *            là từ khóa tìm kiếm hồ sơ
	 * @return trả về số lượng các hồ sơ theo từ khóa
	 * @throws SystemException
	 *             Nếu ngoại lệ hệ thống xảy ra
	 */
	public int countDossier(long groupId, String keywords)
	    throws SystemException {

		return dossierFinder.countDossier(groupId, keywords);
	}

	/**
	 * <p> Đếm số lượng các hồ sơ theo trạng thái hồ sơ </p>
	 * 
	 * @param groupId
	 * @param dossierStatus
	 * @return trả về số lượng các hồ sơ theo trạng thái hồ sơ
	 * @throws SystemException
	 *             Nếu ngoại lệ hệ thống xảy ra
	 */
	public int countDossierByDossierStatus(long groupId, long dossierStatus)
	    throws SystemException {

		return dossierFinder.countDossierByDossierStatus(groupId, dossierStatus);
	}
	 ...
	 * @param dossierStatus
	 *            trạng thái hồ sơ
	 * @return trả về danh sách các đối tượng Dossier theo cấp xử lý hành chính
	 * @throws SystemException
	 *             Nếu ngoại lệ hệ thống xảy ra
	 * @throws NoSuchDossierException
	 *             Khi xảy ra lỗi không tìm thấy Dossier
	 */
	public List<Dossier> searchDossierByAdministrationAndDomain(
	    long groupId, String administrationCode, String domainCode,
	    long dossierStatus, int start, int end)
	    throws SystemException, NoSuchDossierException {

		return dossierFinder.searchDossierByAdministrationAndDomain(
		    groupId, administrationCode, domainCode, dossierStatus, start, end);
	}
	 ...
	 * @param dossierStatus
	 *            là trạng thái hồ sơ cần tìm kiếm
	 * @return trả về số lượng các hồ sơ theo cấp hành chính thực hiện
	 * @throws SystemException
	 *             Nếu ngoại lệ hệ thống xảy ra
	 */
	public int countDossierByAdministrationAndDomain(
	    long groupId, String administrationCode, String domainCode,
	    long dossierStatus)
	    throws SystemException {

		return dossierFinder.countDossierByAdministrationAndDomain(
		    groupId, administrationCode, domainCode, dossierStatus);
	}
	...
	public Dossier addDossier(
		...
		int count =
		    dossierLocalService.countByGroupId(serviceContext.getScopeGroupId());
		...
		for (Map.Entry<String, List<String>> entry : groupPrivateDossier.entrySet()) {

			String groupName = entry.getKey();
			List<String> dossierSchemas = entry.getValue();
			// Add file group
			FileGroup fileGroup = null;
			if (Validator.isNotNull(groupName) && dossierSchemas != null &&
			    !dossierSchemas.isEmpty()) {
				...
				if (Validator.isNotNull(groupName)) {
					...
					dossierStatusLocalService.addDossierStatus(
					    userId, dossierId, fileGroup.getFileGroupId(),
					    PortletConstants.DOSSIER_STATUS_NEW,
					    PortletUtil.getActionInfo(
					        PortletConstants.DOSSIER_STATUS_NEW,
			...
			if (fileGroup != null) {
				for (String dossierSchema : dossierSchemas) {
					...
					jsonObject =
					    JSONFactoryUtil.createJSONObject(dossierSchema);
					long dossierPartId =
					    jsonObject.getLong(DossierFileDisplayTerms.DOSSIER_PART_ID);
					long fileEntryId =
					    jsonObject.getLong(DossierFileDisplayTerms.FILE_ENTRY_ID);

					int dossierFileOriginal =
					    jsonObject.getInt(DossierFileDisplayTerms.DOSSIER_FILE_ORIGINAL);
					int dossierFileType =
					    jsonObject.getInt(DossierFileDisplayTerms.DOSSIER_FILE_TYPE);
					String formData =
					    jsonObject.getString(DossierFileDisplayTerms.FORM_DATA);

					String displayName =
					    jsonObject.getString(DossierFileDisplayTerms.DISPLAY_NAME);
					String mimeType =
					    jsonObject.getString(DossierFileDisplayTerms.MIME_TYPE);
					/*
					 * String fileName = jsonObject
					 * .getString(DossierFileDisplayTerms.FILE_NAME);
					 */
					String dossierFileDate =
					    jsonObject.getString(DossierFileDisplayTerms.DOSSIER_FILE_DATE);
					String dossierFileNo =
					    jsonObject.getString(DossierFileDisplayTerms.DOSSIER_FILE_NO);
					...
					if (fileEntryId > 0) {
						...
						fileEntry =
						    DLAppServiceUtil.addFileEntry(
						        ...
						        dossierNoFolder.getFolderId(), sourceFileName,
					...
					if (fileEntry != null) {
						...
						if (Validator.isNotNull(dossierFileDate)) {
							fileDate =
							    DateTimeUtil.convertStringToDate(dossierFileDate);
						}
						...
						dossierFileLocalService.addDossierFile(
						    ...
						    dossierPartId,
						    templateFileNo,
						    fileGroup != null ? fileGroup.getFileGroupId() : 0L,
		...
		for (int i = 0; i < normalDossierSchemas.size(); i++) {
			String schema = normalDossierSchemas.get(i);

			JSONObject jsonObject = null;

			jsonObject = JSONFactoryUtil.createJSONObject(schema);
			long dossierPartId =
			    jsonObject.getLong(DossierFileDisplayTerms.DOSSIER_PART_ID);
			long fileEntryId =
			    jsonObject.getLong(DossierFileDisplayTerms.FILE_ENTRY_ID);
	...
	public Dossier updateDossierStatus(
		...
		DossierStatus dossierStatus =
		    dossierStatusLocalService.getDossierStatus(dossierId);
		dossierStatus =
		    getDossierStatus(
		...
		if (fileGroupId > 0) {
			...
			List<DossierFile> dossierFiles =
			    dossierFileLocalService.getDossierFileByD_F(
		...
		dossierLogLocalService.addDossierLog(
		    ...
		    fileGroupId, status, PortletUtil.getAccountStatus(status, locale),
	...
	protected Dossier getDossier(
	    Dossier dossier, long userId, long govAgencyOrganizationId, int status) {
	...
	public Dossier updateDossier(
	    ...
	    long dossierFolderId, ServiceContext serviceContext)
	    throws SystemException, PortalException {
				...
				for (String dossierSchema : dossierSchemas) {
					...
					if (fileEntryId > 0) {
						...
						fileEntry =
						    DLAppServiceUtil.addFileEntry(
						        serviceContext.getScopeGroupId(),
						        dossierFolderId, sourceFileName, mimeType,
		...
		for (int i = 0; i < normalDossierSchemas.size(); i++) {
			...
			if (fileEntryId > 0) {
				...
				fileEntry =
				    DLAppServiceUtil.addFileEntry(
				        serviceContext.getScopeGroupId(), dossierFolderId,
	...
	public Dossier fetchByF_ReceptionNo(long groupId, String receptionNo)
	    throws SystemException {
